...
export default function ManageProducts() {
  ...
  useEffect(async() =>{ //Haetaan tuotteet backendistä
    ...
    const deleteProduct = async(idTuote) => { 
        ...
        await fetch(`http://localhost:3000/tuote/${idTuote}`, { method: 'DELETE'}) //Poistetaan tuote tietokannasta
        newProds.splice(deletedProd, 1);  //Poistetaan tuote näkymästä
        setProducts(newProds);  //Päivitetään näkymä poiston jälkeen
    }

    const createProduct = async(item) => {
        await fetch(`http://localhost:3000/tuote`,{ method: 'POST',
        ...
        const resultTuote = await fetch(`http://localhost:3000/tuote`).then((res) => //Päivitetään näkymä lisäyksen jälkeen
        res.json()
        )
